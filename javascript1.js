    ...
    // Abre enlace del catálogo (ajusta el href si tienes URL real)
    catalogoBtn.addEventListener('click', (e)=>{
      e.preventDefault();
      // Si tienes una ruta real, reemplaza '#' por la URL o dispara la acción correspondiente
      window.open('catalogo.html', '_blank'); // <--- cambiar aquí si hace falta
      // También podemos enviar un mensaje automático explicando que está en el botón
    ...
    // boton ver catálogo dentro de sugerencias
    ...
    function respuestaBot(mensaje, usuario){
      ...
      // Mapa de respuestas más importantes (no exhaustivo)
      const map = {
        ...
        '¿CÓMO HAGO UN PEDIDO?': `Elige un producto en el botón "Catálogo" y elige un producto`,
      ...
      if(msg.includes('PELUCHE') || msg.includes('PELUCHE') ) return map['¿QUÉ TAMAÑOS DE PELUCHES TIENEN?'];
